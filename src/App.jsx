...
// --- LOGO IMPORT (Public folder se uthayega) ---
// Image: public/college_logo.png
...
const Icons = {
    ...
    Refresh: () => <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
};
...
const App = () => {
    ...
    // --- HARD RESET PASSWORD LOGIC (RE-REGISTER) ---
    ...
    const handleHardResetPassword = async (e) => {
        ...
        const confirmMsg = "IMPORTANT: For this to work, you MUST have already deleted this user from Firebase Console > Authentication.\n\nHave you deleted the user from the Console?";
        ...
        try {
            // 1. Re-create the user in Auth (This will link to existing Firestore Doc)
            await createUserWithEmailAndPassword(auth, email, password);
            
            // 2. Sign Out immediately (Because we are Admin)
            await signOut(auth);
            
            // 3. Re-login as Admin automatically (Optional, but good UX)
            // Note: In real app, we might ask admin to login again.
            ...
            if(error.code === 'auth/email-already-in-use') {
                alert("Error: User still exists in Firebase Console.\nPlease go to Firebase Console -> Authentication and DELETE this user first.");
            } else {
            ...
            <div className="min-h-screen flex items-center justify-center bg-[#f1f5f9] p-6"> 
                <div className="bg-white rounded-2xl shadow-xl border border-gray-200 w-full max-w-md overflow-hidden"> 
                    <div className="bg-[#007cb7] p-10 text-center flex flex-col items-center"> 
                        <img src="/college_logo.png" alt="Logo" className="h-20 mb-4 object-contain rounded-md bg-white p-2" />
            ...
            <div className="min-h-screen bg-slate-50 font-sans text-gray-700"> 
                <header className="bg-[#007cb7] border-b border-[#006391] h-16 flex items-center justify-between px-8 sticky top-0 z-30 shadow-md"> 
                    <div className="flex items-center gap-3">
                         <img src="/college_logo.png" alt="Logo" className="h-10 w-auto object-contain bg-white rounded p-0.5" />
                        <h2 className="text-lg font-bold text-white tracking-wide">ADMINISTRATION</h2>
                    </div> 
                ...
                <main className="p-8 max-w-7xl mx-auto space-y-8"> 
                    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm"> 
                        ...
                        <div className="flex gap-2"> <input type="text" className="flex-1 px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-[#007cb7] outline-none" placeholder="Search by CNIC..." value={adminSearchCnic} onChange={(e) => setAdminSearchCnic(e.target.value)} /> <button onClick={handleAdminSearch} className="bg-[#007cb7] hover:bg-[#006391] text-white px-6 py-2 rounded font-bold text-sm">SEARCH</button> </div> 
                        {searchedStudent && ( 
                    ...
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                        <div className="bg-white rounded-lg shadow-xl w-full max-w-sm overflow-hidden">
                            <div className="p-5 border-b bg-orange-50">
                                <h3 className="font-bold text-orange-700">Hard Reset Password</h3>
                                <p className="text-xs text-orange-600 mt-1">
                                    Only for Fake Emails!
                                </p>
                            </div>
                            <form onSubmit={handleHardResetPassword} className="p-5 space-y-4">
                                <div className="text-xs text-gray-600 bg-gray-100 p-3 rounded">
                                    <strong>Step 1:</strong> Go to Firebase Console &gt; Auth.<br/>
                                    <strong>Step 2:</strong> Delete User <code>{getEmail(searchedStudent.cnic)}</code>.<br/>
                                    <strong>Step 3:</strong> Enter NEW password below.
                                </div>
                                <div>
                                ...
                                <div className="flex justify-end gap-2">
                                    <button type="button" onClick={() => setShowResetModal(false)} className="px-4 py-2 text-gray-500 hover:bg-gray-100 rounded text-sm font-bold">CANCEL</button>
                                    <button type="submit" className="px-4 py-2 bg-orange-600 text-white rounded text-sm font-bold hover:bg-orange-700">RESET & RE-REGISTER</button>
                                </div>
                            </form>
                        </div>
                    </div>
                ...
                {/* ... (Edit & Receipt Modals - same as before) ... */}
        ...
        <>
            ...
            <div className="flex h-screen bg-[#f8fafc] font-sans text-gray-800">
                ...
                <aside className="w-64 bg-[#007cb7] text-white flex flex-col shadow-2xl hidden md:flex sticky top-0 h-screen">
                    <div className="h-24 flex items-center justify-center border-b border-[#006391] bg-[#006391] px-4">
                        <img src="/college_logo.png" alt="College Logo" className="max-h-full max-w-full object-contain rounded bg-white p-1" />
                    </div>
                    <div className="p-6">
