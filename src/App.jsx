...
// --- Icons ---
const Icons = {
    Dashboard: ({className}) => <svg className={className || "w-5 h-5"} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>,
    Profile: ({className}) => <svg className={className || "w-5 h-5"} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>,
    Exam: ({className}) => <svg className={className || "w-5 h-5"} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>,
    Logout: ({className}) => <svg className={className || "w-5 h-5"} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>,
    ...
    Upload: ({className}) => <svg className={className || "w-4 h-4 mr-2"} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>,
...
const App = () => {
    ...
    const handleAuth = async (e) => { e.preventDefault(); setErrorMsg(''); setIsAuthLoading(true); const input = authInput.cnic.trim(); let password = authInput.password.trim(); let email; if (input.includes('@')) { email = input; } else { email = `${input}@studentportal.com`; } if (input !== 'admin' && !input.includes('@')) { if (input !== password) { setIsAuthLoading(false); return setErrorMsg("Password must be same as CNIC."); } } try { await signInWithEmailAndPassword(auth, email, password); } catch (loginError) { try { if (!input.includes('@')) { await createUserWithEmailAndPassword(auth, email, password); if (input !== 'admin') { const initialData = { cnic: input, personalInfo: { ...studentForm, cnic: input }, challans: [] }; await setDoc(doc(db, "students", input), initialData); } } else { throw loginError; } } catch (registerError) { setIsAuthLoading(false); if (registerError.code === 'auth/weak-password') setErrorMsg("Error: Password too short."); else setErrorMsg("Error: " + registerError.message); } } };
        ...
        <> 
            ...
            <div className="min-h-screen flex items-center justify-center bg-[#f1f5f9] p-6"> 
                <div className="bg-white rounded-2xl shadow-xl border border-gray-200 w-full max-w-md overflow-hidden"> 
                    <div className="bg-[#1a4d0f] p-6 text-center flex flex-col items-center"> 
                        <img src="../public/logo1.png" alt="Logo" className="h-20 mb-4 object-contain" />
                        <h1 className="text-xl font-bold text-white tracking-widest uppercase">GOVT. ELEMENTARY COLLEGE</h1>
                        <h2 className="text-sm font-bold text-yellow-400 tracking-wider">OF EDUCATION (M/W) MITHI</h2>
                        <p className="text-gray-200 text-[10px] mt-1">Education & Literacy Dept, Govt. of Sindh</p>
                    </div> 
                    <div className="p-10"> 
                        <form onSubmit={handleAuth} className="space-y-6"> 
                            <div><label className="block text-gray-500 text-xs font-bold mb-2 uppercase tracking-wide">CNIC / Admin User / Email</label><input type="text" className="w-full px-4 py-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#1a4d0f] focus:border-transparent outline-none bg-gray-50" placeholder="e.g. 44303... or email@gmail.com" value={authInput.cnic} onChange={(e) => setAuthInput({...authInput, cnic: e.target.value})} required /></div> 
                            <div><label className="block text-gray-500 text-xs font-bold mb-2 uppercase tracking-wide">Password</label><input type="password" className="w-full px-4 py-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#1a4d0f] focus:border-transparent outline-none bg-gray-50" placeholder="••••••••" value={authInput.password} onChange={(e) => setAuthInput({...authInput, password: e.target.value})} required /></div> 
                            ...
                            <button type="submit" className="w-full bg-[#1a4d0f] hover:bg-[#12360a] text-white font-bold py-3.5 rounded-md shadow-md transition-all uppercase tracking-wide text-sm flex justify-center items-center"> {isAuthLoading ? <Icons.Spinner /> : "Sign In"} </button> 
                        </form> 
                    </div> 
                </div> 
            </div> 
    ...
    if (isAdmin) return (
        <> 
            ...
            <div className="min-h-screen bg-slate-50 font-sans text-gray-700"> 
                {/* --- ADMIN HEADER --- */}
                <header className="bg-[#1a4d0f] border-b border-green-800 h-20 flex items-center justify-between px-8 sticky top-0 z-30 shadow-md"> 
                    <div className="flex items-center gap-4">
                        <img src="/college_logo.png" alt="Logo" className="h-14 w-auto object-contain bg-white rounded p-1" />
                        <div>
                            <h2 className="text-sm font-bold text-white tracking-wide">GOVT. ELEMENTARY COLLEGE</h2>
                            <h3 className="text-xs font-bold text-yellow-400 tracking-wide">OF EDUCATION (M/W) MITHI</h3>
                        </div>
                    </div> 
                    <button onClick={handleLogout} className="text-white hover:bg-green-700 px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors"><Icons.Logout /> LOGOUT</button> 
                </header> 
                <main className="p-8 max-w-7xl mx-auto space-y-8"> 
                    {/* ... (Admin Panel Content Same as Before) ... */}
    ...
    return (
        <>
            ...
            <div className="flex h-screen bg-[#f8fafc] font-sans text-gray-800">
                {/* Sidebar */}
                <aside className="w-full md:w-64 bg-[#1a4d0f] text-white flex-col shadow-2xl hidden md:flex sticky top-0 h-screen">
                    <div className="h-24 flex items-center justify-center border-b border-green-800 bg-[#1a4d0f] px-4">
                        {/* --- SIDEBAR HEADER --- */}
                        <div className="flex flex-col items-center">
                            <img src="../public/logo.png" alt="College Logo" className="h-10 w-auto object-contain bg-white rounded p-0.5 mb-1" />
                            <h1 className="text-[10px] font-extrabold text-white leading-tight tracking-wider text-center">GOVT. ELEMENTARY COLLEGE</h1>
                            <h2 className="text-[8px] font-bold text-yellow-400 leading-tight tracking-wider text-center">OF EDUCATION (M/W) MITHI</h2>
                        </div>
                    </div>
                    <div className="p-6">
                        <div className="flex items-center gap-3 mb-8 p-3 bg-green-900/50 rounded-lg border border-green-800">
                            <div className="w-8 h-8 rounded-full bg-white text-[#1a4d0f] flex items-center justify-center text-xs font-bold overflow-hidden border-2 border-green-200">
                            ...
                            <div className="overflow-hidden"><h4 className="text-xs font-bold truncate text-white uppercase">{studentForm.fullName || 'Student'}</h4><p className="text-[10px] text-green-200 truncate">{user.email.split('@')[0]}</p></div>
                        </div>
                        <nav className="space-y-1">
                            {[
                                ...
                                <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center px-4 py-3 rounded-lg transition-all duration-200 ${activeTab === item.id ? 'bg-white text-[#1a4d0f] shadow-md font-bold' : 'text-green-100 hover:bg-green-800 hover:text-white font-medium'}`}>
                    ...
                    <div className="mt-auto p-6 border-t border-green-800">
                        <button onClick={handleLogout} className="w-full flex items-center justify-center px-4 py-3 rounded-lg text-green-100 hover:bg-green-800 hover:text-white transition-colors font-bold text-xs uppercase tracking-wide"><Icons.Logout /> LOGOUT</button>
                    </div>
                </aside>

                {/* Main Content */}
                <div className="flex-1 flex flex-col overflow-hidden h-screen">
                    <header className="h-16 bg-[#1a4d0f] border-b border-green-800 flex items-center justify-between px-8 sticky top-0 z-20 shadow-md">
                        ...
                        <div className="text-xs text-green-100 font-medium uppercase tracking-wider">{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div>
                    </header>
                        ...
                        {activeTab === 'dashboard' && (
                            <div className="animate-fade-in space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-100 flex items-center justify-between"><div><p className="text-xs text-gray-400 font-bold uppercase mb-1">Total Challans</p><h3 className="text-2xl font-bold text-gray-800">{userData?.challans?.length || 0}</h3></div><div className="p-2 bg-green-50 text-[#1a4d0f] rounded"><Icons.Exam /></div></div>
                                                    ...
                                                    {userData.challans.map((c) => (
                                                        <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                                                            ...
                                                            <td className="px-6 py-4 text-xs font-bold flex items-center gap-3">
                                                                <button onClick={() => handleDownloadPdf(c)} className="text-[#1a4d0f] hover:underline flex items-center gap-1"><Icons.Download /> PDF</button>
                                                                {c.status === 'Verified' ? (
                                                                    ...
                                                                    <button onClick={() => openUploadModal(c.id)} className="bg-[#1a4d0f] text-white px-3 py-1.5 rounded hover:bg-[#12360a] flex items-center gap-1 shadow-sm transition-all"><Icons.Upload /> UPLOAD</button>
                                                                ))}
                                                            </td>
                                                        </tr>
                                                    ))}
                                ...
                                {isEditing ? (
                                    <form onSubmit={saveProfile} className="p-8">
                                        <div className="flex flex-col items-center mb-8">
                                            <div className="w-56 h-56 rounded-[2rem] bg-gray-100 mb-3 overflow-hidden border-4 border-white shadow-md relative group">
                                            ...
                                            <label className="mt-3 bg-[#1a4d0f] hover:bg-[#12360a] text-white px-4 py-2 rounded text-xs font-bold uppercase tracking-wide cursor-pointer shadow-sm transition-all flex items-center gap-2">
                                    ...
                                    <div className="p-8 flex flex-col items-center relative">
                                        <div className="flex flex-col items-center mb-8">
                                            <div className="w-56 h-56 rounded-[2rem] bg-gray-100 mb-4 overflow-hidden border-4 border-white shadow-lg">
                                                {studentForm.profileImage ? <img src={studentForm.profileImage} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-gray-300"><Icons.Profile /></div>}
                                            </div>
                                        ...
                                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 w-full max-w-2xl overflow-hidden mb-8">
                                            <div className="grid grid-cols-1 md:grid-cols-2 divide-y divide-gray-100 md:divide-y-0 md:divide-x">
                                                 ...
                                                 <div className="p-5 space-y-4 bg-gray-50/50">
                                                     ...
                                                     <div><h5 className="text-[10px] font-bold text-[#1a4d0f] uppercase tracking-wider mb-1">Email Address</h5><p className="text-sm font-bold text-[#1a4d0f] lowercase">{studentForm.email || 'No Email Added'}</p></div>
                                        ...
                                        <div className="flex gap-4">
                                            <button onClick={() => setIsEditing(true)} className="bg-[#1a4d0f] hover:bg-[#12360a] text-white px-8 py-3 rounded-lg font-bold shadow-md transition-all text-xs uppercase tracking-wide flex items-center gap-2">
                        ...
                        {activeTab === 'generator' && (
                            <div className="max-w-xl mx-auto mt-10">
                                <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden animate-fade-in">
                                    <div className="bg-[#1a4d0f] px-6 py-5 text-center"><h3 className="text-lg font-bold text-white tracking-wide uppercase">Fee Challan Generator</h3></div>
                                        ...
                                        <form onSubmit={(e) => { e.preventDefault(); generateChallan(e.target.part.value, e.target.batch.value, e.target.status.value); }}>
                                            <div className="space-y-4">
                                                <div>
                                                    ...
                                                    <select name="part" className="w-full px-4 py-3 border border-gray-300 rounded focus:ring-1 focus:ring-[#1a4d0f] outline-none bg-gray-50 text-sm"><option value="I">Part I (First Year)</option><option value="II">Part II (Second Year)</option><option value="III">Part III (Third Year)</option><option value="IV">Part IV (Fourth Year)</option></select>
                                                </div>
                                                <div>
                                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Select Batch</label>
                                                    <select name="batch" className="w-full px-4 py-3 border border-gray-300 rounded focus:ring-1 focus:ring-[#1a4d0f] outline-none bg-gray-50 text-sm"><option value="2k22">Batch 2k22</option><option value="2k23">Batch 2k23</option><option value="2k24">Batch 2k24</option><option value="2k25">Batch 2k25</option><option value="2k26">Batch 2k26</option></select>
                                                </div>
                                                <div>
                                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Resident Status</label>
                                                    <select name="status" className="w-full px-4 py-3 border border-gray-300 rounded focus:ring-1 focus:ring-[#1a4d0f] outline-none bg-gray-50 text-sm"><option value="Hosteller">Hosteller (Boarder) - Rs.1300</option><option value="Non-Hosteller">Non-Hosteller (Day Scholar) - Rs.900</option></select>
                                                </div>
                                            </div>
                                            <button type="submit" className="w-full mt-6 bg-[#1a4d0f] hover:bg-[#12360a] text-white font-bold py-3 rounded shadow-lg transition-all text-xs uppercase tracking-wide flex justify-center items-center gap-2"><Icons.Download /> Generate PDF</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
